...
**Applies to:** Step01 (S1) → Step02 (S2) → Step03 (S3) → Step04 (S4) → Step05 (S5) → Step06 (S6)
**Compliance:** MI-CLEAR-LLM 2025
**Last Updated:** 2025-12-26 (v1.6: S5 Validation 추가, S5 Export → S6으로 변경)
...
Step02 (S2): Execution-only Card Generation
        ↓
Candidate Cards (Q1, Q2 per entity; 2-card policy)
        ↓
...
Step04 (S4): Image Generation & Manifest Creation
        ↓
Rendered Images (JPG files)
        ↓
Step05 (S5): Validation & Triage (LLM-based content quality validation)
        ↓
Validation Results (S5 validation artifacts)
        ↓
Step06 (S6): Export (PDF Builder for S0 QA / Anki Deck Packaging)
        ↓
Distribution Artifacts
```
Pipeline Overview (One-line)
S1 (Structure-only entity definition) → S2 (Exact-N execution) → S3 (State-only compiler: policy resolution & image spec) → S4 (Render-only image generation) → S5 (Validation & triage) → S6 (Packaging: PDF/Anki)
...
1. `len(anki_cards) == N` and **N MUST be exactly 2** (2-card policy: Q1, Q2)
2. Text-only output (no image files)
3. `image_hint` field is **REQUIRED** for both Q1 and Q2 (back-only infographics)
4. No entity reinterpretation
5. No policy inference

**Card Roles:**
- Q1: BASIC card with `image_hint` (required)
- Q2: MCQ card with `image_hint` (required)
...
2. **Image Specifications:**
   - Card images: `aspect_ratio="4:5"`, `image_size="2K"` (default, configurable via `S4_CARD_IMAGE_SIZE`)
   - Table visuals: `aspect_ratio="16:9"`, `image_size="4K"`
3. **Deterministic Filenames:**
   - Card images: `IMG__{run_tag}__{group_id}__{entity_id}__{card_role}.jpg`
   - Table visuals: `IMG__{run_tag}__{group_id}__TABLE.jpg`
4. **Fail-Fast Rules:**
   - Q1 image generation failure → FAIL-FAST
   - Q2 image generation failure → FAIL-FAST
...
* S1 structure (`stage1_struct__arm{X}.jsonl`)
* S2 results (`s2_results__arm{X}.jsonl`)

### 7.3 Hard Rules (Binding)

1. **LLM Model Configuration (Arm-Independent):**
   - S1 table validation: Pro model, `thinking=True`, `rag_enabled=True` (1 call per table)
   - S2 card validation: Flash model, `thinking=True`, `rag_enabled=True` (1 call per card)
   - Same model configuration for all arms (fixed across arms)

2. **Validation Output:**
   - Blocking error flags (Technical Accuracy = 0.0)
   - Quality scores (Technical Accuracy 0/0.5/1, Educational Quality 1-5)
   - Issues list with RAG evidence (when blocking errors flagged)

3. **No content modification** (read-only validation)
4. **No fail-fast stopping** (pipeline continues regardless of validation results)
5. **RAG evidence required** when blocking error is flagged

### 7.4 Output

* `s5_validation__arm{X}.jsonl` (validation results with `s5_snapshot_id` for reproducibility)

**Reference:** `04_Step_Contracts/Step05_S5/S5_Validation_Contract_Canonical.md`

---

## 8. Step06 (S6): Export & Packaging

### 8.1 Authoritative Definition

**Step06 (S6) consists of two export tools: PDF Builder for S0 QA and Anki Deck Exporter.**

---

### 8.2 PDF Builder (S0 QA)
...
**Output:**
* `SET_{group_id}_arm{arm}_{run_tag}.pdf` (non-blinded)
* `SET_{surrogate}_{run_tag}.pdf` (blinded mode)
...
**Output:**
* `MeducAI_{run_tag}_arm{X}.apkg`

**Image Placement (Hardcoded):**
* Q1: Image in Back (back-only infographic)
* Q2: Image in Back (back-only infographic)
...
| Content validation | S5 (triage/flagging) |
| Packaging         | S6                |
