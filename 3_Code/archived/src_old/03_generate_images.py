...
def extract_first_image_bytes(response: Any) -> Optional[bytes]:
    ...
    # 1) direct parts
    b = _try_parts(getattr(response, "parts", None))
    if b:
        return b
    ...
    try:
        ...
        parts2 = cand0.content.parts
        return _try_parts(parts2)
    except Exception:
        return None
...
def main():
    ...
    out_dir = IMAGE_ROOT / args.provider / args.run_tag / input_kind
    ensure_dir(out_dir)
    ...
    if args.model.strip():
        ...
        # P0-4 support: allow different default models by kind via ENV
        if input_kind == "table" and IMAGE_MODEL_TABLE:
            model_id = IMAGE_MODEL_TABLE
        elif input_kind == "entity" and IMAGE_MODEL_ENTITY:
            model_id = IMAGE_MODEL_ENTITY
        else:
            model_id = MODEL_CONFIG[args.provider]["model_name"]
