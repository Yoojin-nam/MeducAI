"""
04_export_anki.py (MeducAI v3.7 - integrated path)
--------------------------------
...
- [UPDATES v3.7] Path lookup changed to unified run_tag folder for CSV and Image Manifest.
"""
...
from generated_paths import generated_image_dir
...
def attach_image_html(
    ...
    if not p.exists():
        # NOTE: The path points to the unified image folder generated/<run_tag>/entity/
        # If the file is missing, it means 03_generate_images failed or was not run.
        return ""
...
def main() -> None:
    ...
    # [MODIFIED v3.7] CSV path lookup changed to unified resolver
    if args.input_csv.strip():
        in_csv = Path(args.input_csv).expanduser().resolve()
        if not in_csv.is_absolute():
            # Legacy path fallback logic is removed here; rely on external helper
            raise FileNotFoundError(f"Input CSV must be absolute or full path if specified: {args.input_csv}")
    else:
        # Assumes selected CSV exists.
        from generated_paths import resolve_in_run_or_legacy
        filename = f"anki_cards_selected_{args.provider}_{args.run_tag}.csv"
        in_csv = resolve_in_run_or_legacy(base_dir, args.run_tag, args.provider, filename)
    ...
    # [MODIFIED v3.7] Entity manifest path now uses the unified image directory helper
    entity_image_root = generated_image_dir(base_dir, args.run_tag, "entity")
    entity_manifest = entity_image_root / f"manifest_{args.provider}_{args.run_tag}_entity.jsonl"
