"""
04_5_check_deck_stats.py (MeducAI v3.7 - integrated path)
---------------------------------------
Purpose:
...
Input:
- 2_Data/metadata/generated/<run_tag>/anki_cards_<provider>_<run_tag>.csv (candidate) OR
- 2_Data/metadata/generated/<run_tag>/anki_cards_selected_<provider>_<run_tag>.csv (selected)

Outputs:
- prints a concise report to stdout
- optionally writes JSON report to 2_Data/metadata/generated/<run_tag>/deck_stats_<provider>_<run_tag>.json
- [UPDATES v3.7] Path lookup changed to unified run_tag folder.
"""
...
def parse_mix(s: str) -> Dict[str, float]:
    ...
    for part in s.split(","):
        part = part.strip()
        if not part or ":" in part: # Fix: if ":" is missing, assume it's just a type name or invalid.
            continue
        k, v = part.split(":", 1)
        k = k.strip()
        try:
...
def main() -> None:
    ...
    # [MODIFIED v3.7] Use resolve_in_run_or_legacy which now points to the unified path.
