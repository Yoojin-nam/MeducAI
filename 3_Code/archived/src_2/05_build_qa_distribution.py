...
- [UPDATES v3.7] All paths use unified <run_tag> directory structure (Provider folder removed).
...
# [NEW] Import path helper
from generated_paths import generated_run_dir, generated_image_dir, resolve_in_run_dir
...
def guess_csv_paths(base_dir: Path, provider: str, run_tag_base: str, arm: str) -> Tuple[Path, Path, Path]:
    ...
    """
    run_dir = generated_run_dir(base_dir, run_tag_base)
    arm_suffix = f"__arm{arm}"
    
    # Filenames still contain provider for audit
    image_prompts = run_dir / f"image_prompts_{provider}_{run_tag_base}{arm_suffix}.csv"
    table_prompts = run_dir / f"table_infographic_prompts_{provider}_{run_tag_base}{arm_suffix}.csv"
    selected_cards = run_dir / f"anki_cards_selected_{provider}_{run_tag_base}{arm_suffix}.csv"
    return image_prompts, table_prompts, selected_cards

def manifest_path(base_dir: Path, provider: str, run_tag: str, kind: str) -> Path:
    """
    03_generate_images.py convention (v3.7):
      2_Data/images/generated/<run_tag>/<kind>/manifest_<provider>_<run_tag>_<kind>.jsonl
    kind: "entity" or "table"
    """
    root = generated_image_dir(base_dir, run_tag, kind)
...
def mirror_entity_manifest_for_export_provider(
    ...
    """
    If export_provider != image_provider, 04_export_anki.py will look for manifest 
    under export_provider/run_tag/entity (LEGACY). 
    
    V3.7 UPDATE: 
    Since the manifest now lives in <run_tag>/entity/ (regardless of image_provider), 
    we must ensure the manifest name matches the export_provider name for 04_export_anki to find it.
    
    04_export_anki.py path expectation: 
      <image_root>/<run_tag>/entity/manifest_<export_provider>_<run_tag>_entity.jsonl
    
    Actual manifest location: 
      <image_root>/<run_tag>/entity/manifest_<image_provider>_<run_tag>_entity.jsonl
      
    We copy the entity manifest, renaming it to match the export_provider name.
    """
    
    # Source manifest (where it was generated)
    src_manifest_name = f"manifest_{image_provider}_{run_tag}_entity.jsonl"
    src_root = generated_image_dir(base_dir, run_tag, "entity")
    src = src_root / src_manifest_name

    if not file_exists(src):
        # NOTE: Using resolve_in_run_dir for robustness, but if image_provider != export_provider 
        # it means the image provider's manifest name won't match.
        # Check the *expected* location based on image provider name
        raise FileNotFoundError(f"Entity manifest not found at: {src} (generated by {image_provider})")

    # Destination manifest (where export_provider's 04_export_anki expects it to be named)
    dst_manifest_name = f"manifest_{export_provider}_{run_tag}_entity.jsonl"
    dst = src_root / dst_manifest_name # Same folder, different name
    
    if dst.exists() and dst.samefile(src):
        # Already mirrored (same file inode); just return dst path
        return dst
