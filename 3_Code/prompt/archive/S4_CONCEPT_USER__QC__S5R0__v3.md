...
- For QC category ONLY: numeric tokens/thresholds/settings MAY appear if and only if they appear anywhere in the master table row content for that entity (not only in 시험포인트).
...
- Text per callout: Entity name + Modality keywords (if present) + Imaging cue token (if extractable) + 시험포인트 keywords (mandatory when non-empty).
