...
5) Image hint requirements:
   ...
   - Q2: OPTIONAL (Q2 reuses Q1 image, so image_hint is not used for image generation but may be provided for consistency).
...
MCQ FORMAT REQUIREMENTS (Q2/Q3) — CRITICAL:
...
- CRITICAL SEPARATION:
  * "front" field: Contains ONLY the question text (e.g., "CT에서 관찰된 소견으로 가장 가능성이 높은 진단은?")
  * "options" array: Contains ONLY the 5 answer choices as separate strings
  * DO NOT put options (①②③④⑤) in the front text. Options must be ONLY in the "options" array field.
- Example for Q2/Q3:
  {
    "card_role": "Q2",
    "card_type": "MCQ",
    "front": "CT axial에서 장골 피질 내 1.2cm 크기의 석회화된 nidus가 관찰되고 주변부 골경화가 현저하다. 가장 가능성이 높은 진단은?",
    "back": "정답: ② Osteoid Osteoma\n\n근거:\n* 1.5cm 미만의 중심부 nidus\n* 주변 반응성 골경화\n* 야간 통증 및 아스피린 반응성",
...
- Q2: image_hint is OPTIONAL (Q2 reuses Q1 image, so image_hint is not used for image generation but may be provided for consistency).
...
- Q1 has non-null image_hint (required)
- Q3 has null or absent image_hint (forbidden)
