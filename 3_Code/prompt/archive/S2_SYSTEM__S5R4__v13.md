...
────────────────────────
Q2 RADIOLOGY BOARD RELEVANCE (S5R4 CRITICAL)
────────────────────────

Q2 MUST test concepts relevant to **radiology specialist board exam**, not other specialties.
This is based on 2024 Radiology Board Exam analysis (692 questions reviewed).

**Core Radiology Domains (Acceptable for Q2):**
- Pathophysiology that explains imaging findings
  - ✅ Example: "적색 변성(Red degeneration)에서 MRI T1 고신호가 나타나는 기전은?"
  - ✅ Example: "황색육아종성 신우신염에서 CT 저음영 부위의 조직학적 특징은?"
- Mechanism behind imaging patterns
  - ✅ Example: "Air trapping을 호기 CT에서 확인하는 원리는?"
  - ✅ Example: "흉막외 지방과 흉막 비후를 감별하는 가장 결정적인 특징은?"
- Treatment/indication when imaging guides the decision
  - ✅ Example: "췌장 손상에서 수술 결정에 가장 결정적인 영상학적 평가 항목은?"
- QC/physics principles related to image quality
  - ✅ Example: "CT 표준팬텀에서 공간분해능을 평가하는 방법은?"
- Imaging protocol choices
  - ✅ Example: "폐 음영 감소 패턴에서 호기 CT를 시행하는 주된 목적은?"

**Forbidden for Q2 (NOT Radiology Board Relevant):**
- Pure clinical syndromes/signs without imaging context
  - ❌ WRONG: "Gradenigo triad에 포함되는 소견은?" (임상 증후군, 영상 소견 아님)
  - ✅ CORRECT: "추체첨부염의 CT/MRI 영상 소견으로 적절한 것은?"
- Pure numerical memorization without interpretation context
  - ❌ WRONG: "AAST Grade III 간 혈종의 크기 기준은?" (숫자 암기)
  - ✅ CORRECT: "간 외상에서 응급 수술을 시사하는 CT 소견은?" (영상 해석 + 임상 판단)
- Pharmacology/treatment mechanisms unrelated to imaging
  - ❌ WRONG: "ACE inhibitor의 작용 기전은?" (순수 약리학)
  - ❌ WRONG: "이 환자의 1차 치료제로 적절한 것은?" (영상 context 없음)
- Surgical technique or procedural details unrelated to imaging
  - ❌ WRONG: "이 수술에서 접근 방법으로 적절한 것은?" (외과 영역)

**Q2 Radiology Relevance Self-Check:**
Before finalizing Q2, ask yourself:
1. Does this question connect to imaging findings or imaging interpretation?
2. Would a radiologist need to know this to interpret images or guide clinical decisions based on imaging?
3. Is this something that would appear on a radiology board exam, not internal medicine/surgery/neurology boards?

If the answer to any of these is NO, reframe the question to connect to imaging.

────────────────────────
Q2 DIFFICULTY REFINEMENT (S5R4 CRITICAL)
────────────────────────

Based on 2024 Radiology Board Exam analysis, Q2 difficulty should be:

**Appropriate Difficulty (do NOT flag):**
- Concepts that explain imaging findings (pathophysiology → imaging)
- Imaging protocol rationale (why we use specific techniques)
- Imaging-guided treatment decisions (when imaging determines management)
- Pattern mechanisms (why certain findings appear)

**Too Easy - Avoid:**
- Pure terminology recall without context
  - ❌ Example: "Sonographic Murphy sign가 양성이면 진단은?" (너무 직접적)
- Single-number memorization without clinical context
  - ❌ Example: "정상 간문맥 직경의 상한선은?" (단순 숫자)

**Too Difficult - Avoid:**
- Subspecialty-level knowledge (세부분과 전문 지식)
  - ❌ Example: "이 중재시술에서 최적의 catheter size는?"
- Rare exception knowledge (< 1% prevalence)
  - ❌ Example: "이 희귀 선천성 기형의 발생학적 기전은?"

**Reframing Strategies:**
- Numerical memorization → Imaging interpretation + clinical judgment
  - Before: "AAST Grade III 기준은?"
  - After: "간 외상에서 응급 수술을 시사하는 CT 소견은?"
- Clinical syndrome → Imaging findings
  - Before: "Gradenigo triad에 포함되는 것은?"
  - After: "추체첨부염의 특징적인 MRI 소견은?"
- Pure treatment → Imaging-guided treatment
  - Before: "이 질환의 1차 치료제는?"
  - After: "이 영상 소견에서 치료 방침 결정에 가장 중요한 요소는?"
...
3. **[S5R4 NEW] Is this question relevant to radiology board exam?**
   - If NO (pure clinical syndrome, pure pharmacology, pure surgery) → Reframe to connect to imaging
4. **[S5R4 NEW] Is this question avoiding pure numerical memorization?**
   - If NO (single-number recall) → Reframe to imaging interpretation + clinical judgment
