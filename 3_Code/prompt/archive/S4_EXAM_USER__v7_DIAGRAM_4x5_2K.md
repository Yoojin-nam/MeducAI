...
MANDATORY IMAGE CONSTRUCTION (DIAGRAM):
...
- The diagram should be consistent with the question and correct answer, but it does NOT need to show every secondary detail.

DIAGRAM STYLE (STRICT):
- Vector-like line art + flat grayscale tone blocks (about 4–6 tones), crisp edges.
- NO photorealistic scan look: no scan noise, no film grain, no fluoroscopy texture, no realistic speckle, no micro-texture.
- Reduce anatomic micro-detail; keep only essential silhouettes and landmarks.

DIDACTIC EMPHASIS (CONTROLLED):
- You MAY make the key finding clearer than a real scan, but keep it clinically plausible.
- Allowed emphasis: gentle outline, subtle tone contrast, one or two arrows/circles pointing to the finding.
- Forbidden emphasis: glowing rims, neon colors, exaggerated size, posterized hard edges, cartoon effects.

ANNOTATION RULES (KEEP TEXT LEGIBLE):
- Prefer arrows/circles first (recommend ≤ 2 pointers).
- Labels are OPTIONAL:
  - Max 2 labels total, each 1–2 words preferred (max 3).
  ...
  - If text is likely to distort, OMIT TEXT and use arrows/circles only.
