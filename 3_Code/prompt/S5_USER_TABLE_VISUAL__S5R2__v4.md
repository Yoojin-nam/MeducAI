# S5R2 — S5 S1 Table + Infographic Validation User Prompt v4.0
...
**⚠️ LANGUAGE POLICY (CRITICAL - READ FIRST):**
- **Korean text is ALLOWED and EXPECTED** in infographics for this Korean medical education context.
- Do NOT penalize Korean text (한글/Hangul) as a prompt compliance violation.
- Do NOT flag Korean text in any zone (headers, entity names, exam points, explanations, takeaways) as an issue.
- The S3 prompt may have mentioned "English-only" constraints, but these are SUPERSEDED by this evaluation policy.
- Focus on **content accuracy and educational value**, NOT language enforcement.
...
1. **Information Clarity** (`information_clarity`): 1-5 Likert
   ...
   - **Note:** Korean text that is correct and legible should NOT be flagged as an issue.
...
3. **Prompt Compliance** (`prompt_compliance`): 0.0 | 0.5 | 1.0
   - Does the infographic match the S3 `infographic_prompt_en` **content requirements** (entities, layout, visual style)?
   - Are all required elements from the prompt visible?
   - **IGNORE language constraints** (English-only rules, ALLOWED_TEXT restrictions) from S3 prompt.
   - Evaluate ONLY: layout structure, entity coverage, visual style compliance.
   - If content is correct but uses Korean instead of English, give full compliance score (1.0).
...
- **Language Policy Reminder**: Korean text is allowed. Do NOT create issues for Korean text usage.
...
**Changes from v3:**
- Added explicit LANGUAGE POLICY allowing Korean text
- Prompt compliance now ignores English-only/ALLOWED_TEXT constraints
- Focus on content accuracy over language enforcement
